    ...
    , _descLabel(gettext("Description:"))
{
    ...
    _table.attach(_nameLabel, 0, 1, 2, 3, Gtk::FILL, Gtk::FILL);
    _table.attach(_nameEntry, 1, 2, 2, 3, Gtk::FILL | Gtk::EXPAND, Gtk::SHRINK);
...
void HardDiskDriveSpecDialog::getValue(HardDiskDriveSpec& spec)
{
    ...
    spec.name = _nameEntry.get_text().c_str();
    spec.description = _descEntry.get_text().c_str();
}
